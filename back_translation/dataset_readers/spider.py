...
class SpiderDatasetReader(DatasetReader):
    def __init__(self,
        ...
        self._keep_if_unparsable_original = keep_if_unparsable
        ...
        self.en2de = torch.hub.load('pytorch/fairseq', 'transformer.wmt19.en-de.single_model', tokenizer='moses',
                               bpe='fastbpe')
    ...
    def text_to_instance(self,
        ...
        # create back-translation:
        if unsup:
            utterance = self.back_translate(utterance)
            db_context = SpiderDBContext(db_id, utterance, tokenizer=self._tokenizer,
                                         tables_file=self._tables_file, dataset_path=self._dataset_path)
            table_field = SpiderKnowledgeGraphField(db_context.knowledge_graph,
                                                    db_context.tokenized_utterance,
            ...
            valid_actions_field = ListField(production_rule_fields)
            fields["valid_actions_bt"] = valid_actions_field
            ...
            action_sequence_field = ListField(index_fields)
            fields["action_sequence_bt"] = action_sequence_field

            fields["utterance_bt"] = TextField(db_context.tokenized_utterance, self._utterance_token_indexers)
            fields["schema_bt"] = table_field
            fields["world_bt"] = MetadataField(world)
        else:
            fields["valid_actions_bt"] = fields["valid_actions"]
            fields["action_sequence_bt"] = fields["action_sequence"]
            fields["utterance_bt"] = fields["utterance"]
            fields["schema_bt"] = fields["schema"]
            fields["world_bt"] = fields["world"]

        return Instance(fields)

    def back_translate(self, utterance: str):
        return self.de2en.translate(self.en2de.translate(utterance))