...
class SpiderParser(Model):
    ...
    def forward(self,  # type: ignore
                ...
                utterance_bt: Dict[str, torch.LongTensor],
                valid_actions: List[List[ProductionRule]],
                valid_actions_bt: List[List[ProductionRule]],
                world: List[SpiderWorld],
                world_bt: List[SpiderWorld],
                schema: Dict[str, torch.LongTensor],
                schema_bt: Dict[str, torch.LongTensor],
                action_sequence: torch.LongTensor = None,
                action_sequence_bt: torch.LongTensor = None) -> Dict[str, torch.Tensor]:
        ...
        # uda - action mask is set to zero in unsup examples
        is_sup = len(torch.nonzero(action_mask.view(-1).data).squeeze()) != 0

        if self.training:
            if is_sup:  # train with supervised examples
                ...
                print("loss_sup", decode_output['loss'])
                return {'loss': decode_output['loss']}
            else:  # uda - train with unsupervised examples
                initial_state = self._get_initial_state(utterance, world, schema, valid_actions)
                initial_state_bt = self._get_initial_state(utterance_bt, world_bt, schema_bt, valid_actions_bt)

                sql_non_bt, action_indices_non_bt = self._get_sql(initial_state, valid_actions)
                sql_bt, action_indices_bt = self._get_sql(initial_state_bt, valid_actions_bt)
                print(action_indices_non_bt[0])
                print(action_indices_bt[0])
                loss_similarity = 0

                for i in range(batch_size):
                    try:
                        similarity = difflib.SequenceMatcher(None, action_indices_non_bt[i], action_indices_bt[i])
                    except:
                        print("fail")
                    loss_similarity += 1 - similarity.ratio()

                loss_similarity = loss_similarity / batch_size
                print("loss_sim", loss_similarity)
                loss = loss_similarity * self._unsup_factor
    ...
    def _get_sql(self, initial_state, actions):
        num_steps = self._max_decoding_steps
        best_final_states = self._beam_search.search(num_steps,
                                                     initial_state,
                                                     self._transition_function,
        ...
        for i in range(batch_size):
            if i not in best_final_states:
                ...
                self._acc_multi(0)
                self._acc_single(0)
                predicted_sql_query_list.append('')
                continue

            best_action_indices = best_final_states[i][0].action_history[0]
            best_action_indices_list.append(best_action_indices)
            ...
            predicted_sql_query_list.append(predicted_sql_query)

        return predicted_sql_query_list, best_action_indices_list
    ...
    def _get_initial_state(self,
        ...
        if self._gnn:
            entities_graph_encoding = self._get_schema_graph_encoding(num_entities, worlds,
